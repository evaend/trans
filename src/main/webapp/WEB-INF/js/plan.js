...
__WEBPACK_IMPORTED_MODULE_7_jQuery___default()('#make').on('click', function () {
  ...
  selectd.map(function (item, index) {
    return opts.push({ id: item.consignNo, text: item.consignNo });
  });
...
__WEBPACK_IMPORTED_MODULE_7_jQuery___default()('#import').on('change', function (e) {
  var files = __WEBPACK_IMPORTED_MODULE_7_jQuery___default()('input[name="exportFile"]').prop('files');
  console.log(files.length);
   console.log('导入记录', files[0]);
  if (files.length === 0) {
    __WEBPACK_IMPORTED_MODULE_5__component_message__["a" /* message */].alert({
      msg: '\u8BF7\u9009\u62E9\u6587\u4EF6'
    });
